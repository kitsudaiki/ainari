...
def create_model(context: AccessContext,
                 name: str,
                 template: str) -> dict:
    path = "/v1alpha/model"
    json_body = {
...
def get_model(context: AccessContext,
              model_uuid: str) -> dict:
    path = f"/v1alpha/model/{model_uuid}"
...
def list_models(context: AccessContext) -> dict:
    path = "/v1alpha/model"
    return ainari_request.send_get_request(context,
                                           context.hanami_address,
                                           path,
...
def delete_model(context: AccessContext,
                 model_uuid: str):
    path = f"/v1alpha/model/{model_uuid}"
...
def delete_all_model(context: AccessContext):
    body = list_models(context)["models"]
    for entry in body:
        delete_model(context, entry["uuid"])
...
def train(context: AccessContext,
    ...
    path = f"/v1alpha/model/{model_uuid}/train"
    json_body = {
        "inputs": inputs,
        "outputs": outputs,
    }
...
def request(context: AccessContext,
            ...
            outputs: list):
    address = f"{context.torii_base_address}:{torii_port}"
    path = f"/v1alpha/model/{model_uuid}/request"
    json_body = {
        "inputs": inputs,
        "outputs": outputs,
    }
