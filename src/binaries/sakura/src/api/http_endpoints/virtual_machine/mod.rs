...
/// 1. Gets image information
/// 2. Downloads the encrypted image
/// 3. Decrypts the image
/// 4. Prepares the image for processing
///
/// # Arguments
/// * `input` - The task input to handle
/// * `endpoint` - The endpoint to use for image operations
/// * `temp_dir` - The temporary directory for file operations
/// * `context` - The user context for authentication
/// * `number_of_cycles` - The number of cycles to potentially adjust
///
/// # Returns
/// * `Result<DataSetFileReadHandle, ErrorResponse>` - The prepared image handle
#[allow(dead_code)]
async fn handle_input(
    ...
    // check if requested column even exist in the image
    if !image_resp.column_names.contains(&input.image_column) {
        let msg = format!(
            "Image-column with name '{}' doesn't exist in image with UUID '{}'",
            input.image_column, input.image_uuid
        );
    ...
    // decrypt image
    let secret = get_secret(&image_resp.secret_uuid, context).await?;
    ...
    file_handle.selected_column = input.image_column.clone();
