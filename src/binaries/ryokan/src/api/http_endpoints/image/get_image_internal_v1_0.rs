...
#[api_operation(
    tag = "image",
    summary = "Get image (internal)",
    description = r###"Get information of an image from the database with additional information."###,
    error_code = 400,
    error_code = 401,
    error_code = 404,
    error_code = 500
)]
pub async fn get_image_internal(
    image_uuid: Path<Uuid>,
    context: UserContext,
) -> Result<Json<ImageInternalResp>, ErrorResponse> {
