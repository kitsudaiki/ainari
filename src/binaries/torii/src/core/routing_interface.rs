...
pub fn init_routing() -> GatewayState {
    ...
    let arp_proxy_map_data = bpf.take_map("ARP_PROXY_MAP").expect("Missing ARP_PROXY_MAP");
    let arp_proxy_map: AyaHashMap<_, u32, ArpProxyPod> = AyaHashMap::try_from(arp_proxy_map_data).unwrap();

    let filter_map_data = bpf.take_map("FILTER_MAP").expect("Missing FILTER_MAP");
    let filter_map: AyaHashMap<_, u32, RouteFilterPod> = AyaHashMap::try_from(filter_map_data).unwrap();
    ...
    if get_ifindex(&overlay_iface) > 0 {
        ...
        println!("Waiting for dynamic TAP creation. Skipping initial overlay attach for {}", overlay_iface);
    }
