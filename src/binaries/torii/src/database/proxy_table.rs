...
pub fn get_proxy(proxy_uuid: &Uuid, _: &UserContext) -> Result<ProxyEntry, enums::DbError> {
    let mut conn = db_handle::DB_CONN.lock().expect("mutex poisoned");
    use self::proxys::dsl::*;
...
pub fn list_proxys() -> QueryResult<Vec<ProxyEntry>> {
    let mut conn = db_handle::DB_CONN.lock().expect("mutex poisoned");
    use self::proxys::dsl::*;

    let query = proxys.filter(status.eq("ACTIVE")).into_boxed();
