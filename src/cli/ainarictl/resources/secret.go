...
var generateSecretCmd = &cobra.Command{
	Use:   "generate SECRET_NAME",
	Short: "Create a new secret with a payload, which is generated by the server.",
	Args:  cobra.ExactArgs(1),
	Run: func(cmd *cobra.Command, args []string) {
		context, err := Login()
		if err != nil {
		...
		secretName := args[0]
		content, err := ainari_sdk.GenerateSecret(context, secretName)
		if err == nil {
			ainarictl_common.PrintSingle(content)
		} else {
			fmt.Println(err)
...
var getSecretCountCmd = &cobra.Command{
	Use:   "count",
	Short: "Get the number of secrets of the project.",
	Run: func(cmd *cobra.Command, args []string) {
		context, err := Login()
		if err != nil {
			fmt.Println(err)
		...
		content, err := ainari_sdk.GetSecretCount(context)
		if err == nil {
			ainarictl_common.PrintSingle(content)
		} else {
			fmt.Println(err)
...
func Init_Secret_Commands(rootCmd *cobra.Command) {
	...
	secretCmd.AddCommand(generateSecretCmd)
	...
	secretCmd.AddCommand(getSecretCountCmd)
}
