...
<template>
    <div class="modal-overlay" @click.self="cancel">
        <div class="modal floating-ip-detach-modal">
            <div class="modal-topbar">
                <span>Detach floating IP</span>
            </div>
            <div class="modal-content">
                <p>Are you sure you want to detach?</p>
                <strong>Floating IP: {{ floating_ip?.floating_ip }}</strong>
                <br />
                <strong>Internal IP: {{ floating_ip?.internal_ip }}</strong>
            </div>
...
async function handleAccept(floating_ip_uuid: string | undefined) {
    ...
    try {
        await hanami.detachFloatingIp(floating_ip_uuid);
        emit("accept");
    } catch (err) {
        errorPopupMsg.value = handleAxiosError(
            err,
            "Failed to detach floating IP",
        );
    }
}
