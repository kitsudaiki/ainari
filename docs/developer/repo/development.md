...
- In file `/etc/ainari/hanami_testing.conf` update the path to the 4 MNIST files, so they point to
...
- Exectute the binary without any flags. It uses by default the config of `/etc/ainari/sakura.toml`
    and the values of the example-config are enough for testing. The SQLite-database-file
    `/etc/ainari/ainari_db` is automatically created by the initial start.
